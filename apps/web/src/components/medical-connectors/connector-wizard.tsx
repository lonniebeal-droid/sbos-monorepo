...
export function ConnectorWizard({ vendors, connectors, disabled }: { vendors: VendorProfile[]; connectors: ConnectorRow[]; disabled?: boolean }) {
  ...
  function test() { if(!existing) return toast.error("Save the connector first"); startTransition(async()=>{ const r=await testMedicalConnectorAction(existing.id); r.ok ? toast.success(String(r.data?.message ?? "FHIR capability test completed")) : toast.error(r.error); location.reload(); }); }
