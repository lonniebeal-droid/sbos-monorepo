...
export default async function MedicalConnectorsPage() {
  ...
  return <>
    ...
    <div className="grid gap-4 xl:grid-cols-[1.25fr_.75fr]">
      ...
      <Card><CardHeader><div className="flex items-center gap-2"><Cable className="h-5 w-5"/><CardTitle>FHIR connection status</CardTitle></div><CardDescription>CapabilityStatement checks only. No patient records are pulled during connection testing.</CardDescription></CardHeader><CardContent className="space-y-3">{connectors.length === 0 ? <p className="text-sm text-muted-foreground">No connector configured yet.</p> : connectors.map(c => <div key={c.id} className="rounded-lg border p-3"><div className="flex items-center justify-between gap-2"><p className="font-medium">{c.vendor}</p><Badge variant={c.status === "connected" ? "success" : c.status === "error" ? "destructive" : c.status === "testing" ? "warning" : "secondary"}>{c.status.replaceAll("_", " ")}</Badge></div><p className="mt-1 break-all text-xs text-muted-foreground">{c.baseUrl}</p><p className="mt-2 text-xs">FHIR: {c.fhirVersion ?? "not verified"}</p>{c.lastError && <p className="mt-1 text-xs text-destructive">{c.lastError}</p>}</div>)}</CardContent></Card>
    </div>
    <EdiStatusPanel />
  </>;
}
