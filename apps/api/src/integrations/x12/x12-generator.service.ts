...
export class X12GeneratorService {
  ...
  async generateInstitutionalClaim837I(claimNumber: string, providerNpi: string, first: string, last: string, dob: Date, memberId: string, icd10Codes: string[], revenueCode: string, billedAmount: number, serviceDate: Date): Promise<string> {
    return this.envelope('837I', `NM1*85*1*PROVIDER*XX*${providerNpi}*NM1*QC*1*${last}^${first}*MI*${memberId}*SV2*${revenueCode}*${billedAmount}*REF*FH*${claimNumber}*DTP*472*D8*${serviceDate.toISOString().slice(0,10)}*IK*${icd10Codes[0] ?? 'Z00.00'}`);
  }
  ...
  parse(x12: string): any {
    ...
    const status=x12.match(/\*L277\*PS\*1\*[^*]+\*([A-Z_]+)/)?.[1];
    const dx=[...x12.matchAll(/\*IK\*([^*]+)/g)].map(m=>m[1]);
    return { transactionSetId: tx, claimNumber: claim, cptCode: cpt, amount: amount ? Number(amount)/100 : undefined, status, icd10Codes: dx };
  }
}
