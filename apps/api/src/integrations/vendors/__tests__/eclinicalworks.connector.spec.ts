...
import { EclinicalworksConnector } from '../eclinicalworks.connector';
import { X12GeneratorService } from '../../x12/x12-generator.service';
import { X12ParserService } from '../../x12/x12-parser.service';

describe('EclinicalworksConnector synthetic parity',()=>{
 let c:EclinicalworksConnector;
 beforeEach(async()=>{ const g=new X12GeneratorService(); c=new EclinicalworksConnector({} as any,{record:async()=>{}} as any,{baseUrl:'https://example.test',tenantId:'tenant-a',clientId:'synthetic'},g,new X12ParserService(g)); await c.authorize('synthetic'); });
 ...
 it('returns synthetic demographics',async()=>{const r:any=await c.getPatientDemographics('1');expect(r.success).toBe(true);expect(r.patient.mrn).toMatch(/^ECWRN/);expect(r.patient.email).toContain('example.test')});
