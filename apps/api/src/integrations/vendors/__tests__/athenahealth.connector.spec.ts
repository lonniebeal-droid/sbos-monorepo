...
import { AthenahealthConnector } from '../athenahealth.connector';
import { X12GeneratorService } from '../../x12/x12-generator.service';
import { X12ParserService } from '../../x12/x12-parser.service';

describe('AthenahealthConnector synthetic parity',()=>{
 let c:AthenahealthConnector;
 beforeEach(async()=>{ const g=new X12GeneratorService(); c=new AthenahealthConnector({} as any,{record:async()=>{}} as any,{baseUrl:'https://example.test',tenantId:'tenant-a',clientId:'synthetic'},g,new X12ParserService(g)); await c.authorize('synthetic'); });
